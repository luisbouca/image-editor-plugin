...
@objcMembers open class CropView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate, CropRectViewDelegate {
    ...
    open var imageCropRect = CGRect.zero {
        didSet {
            ...
            let scale = min(myScrollView.frame.width / imageSize.width, myScrollView.frame.height / imageSize.height)
            let x = imageCropRect.minX * scale + myScrollView.frame.minX
            let y = imageCropRect.minY * scale + myScrollView.frame.minY
            let width = imageCropRect.width * scale
            let height = imageCropRect.height * scale
            
            let rect = CGRect(x: x, y: y, width: width, height: height)
            let intersection = rect.intersection(myScrollView.frame)
    ...
    open var showCroppedArea = true {
        didSet {
            layoutIfNeeded()
            myScrollView.clipsToBounds = !showCroppedArea
    ...
    fileprivate func initialize() {
        ...
        myScrollView = UIScrollView(frame: bounds)
        myScrollView.delegate = self
        myScrollView.autoresizingMask = [.flexibleTopMargin, .flexibleLeftMargin, .flexibleBottomMargin, .flexibleRightMargin]
        myScrollView.backgroundColor = UIColor.clear
        myScrollView.maximumZoomScale = 20.0
        myScrollView.minimumZoomScale = 1.0
        myScrollView.showsHorizontalScrollIndicator = false
        myScrollView.showsVerticalScrollIndicator = false
        myScrollView.bounces = false
        myScrollView.bouncesZoom = false
        ...
        myScrollView.addGestureRecognizer(rotationGestureRecognizer)
    ...
    open override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        let zoomedPoint = CGPoint(x: locationInImageView.x * myScrollView.zoomScale, y: locationInImageView.y * myScrollView.zoomScale)
    ...
    open override func layoutSubviews() {
        ...
        if !resizing {
            layoutCropRectViewWithCropRect(myScrollView.frame)
            if self.interfaceOrientation != interfaceOrientation {
                zoomToCropRect(myScrollView.frame)
            }
        }
    ...
    open func resetCropRectAnimated(_ animated: Bool) {
        ...
        let contentSize = myScrollView.contentSize
        let initialRect = CGRect(x: 0, y: 0, width: contentSize.width, height: contentSize.height)
        myScrollView.zoom(to: initialRect, animated: false)
        
        layoutCropRectViewWithCropRect(myScrollView.bounds)
    ...
    open func zoomedCropRect() -> CGRect {
        let cropRect = convert(myScrollView.frame, to: zoomingView)
    ...
    fileprivate func setupZoomingView() {
        ...
        myScrollView.frame = cropRect
        myScrollView.contentSize = cropRect.size
        
        zoomingView = UIView(frame: myScrollView.bounds)
        zoomingView?.backgroundColor = .clear
        myScrollView.addSubview(zoomingView!)
    }
    ...
    fileprivate func zoomToCropRect(_ toRect: CGRect, shouldCenter: Bool, animated: Bool, completion: (() -> Void)? = nil) {
        if myScrollView.frame.equalTo(toRect) {
            return
        }
        ...
        zoomRect.size.width = cropRect.width / (myScrollView.zoomScale * scale)
        zoomRect.size.height = cropRect.height / (myScrollView.zoomScale * scale)
        ...
        UIView.animate(withDuration: duration, delay: 0.0, options: .beginFromCurrentState, animations: { [unowned self] in
            self.myScrollView.bounds = cropRect
            self.myScrollView.zoom(to: zoomRect, animated: false)
    ...
    fileprivate func cappedCropRectInImageRectWithCropRectView(_ cropRectView: CropRectView) -> CGRect {
        ...
        let rect = convert(cropRect, to: myScrollView)
        if rect.minX < zoomingView!.frame.minX {
            cropRect.origin.x = myScrollView.convert(zoomingView!.frame, to: self).minX
        ...
        if rect.minY < zoomingView!.frame.minY {
            cropRect.origin.y = myScrollView.convert(zoomingView!.frame, to: self).minY
        ...
        if rect.maxX > zoomingView!.frame.maxX {
            let cappedWidth = myScrollView.convert(zoomingView!.frame, to: self).maxX - cropRect.minX
            let height = !keepAspectRatio ? cropRect.size.height : cropRect.size.height * (cappedWidth / cropRect.size.width)
            cropRect.size = CGSize(width: cappedWidth, height: height)
        }
        
        if rect.maxY > zoomingView!.frame.maxY {
            let cappedHeight = myScrollView.convert(zoomingView!.frame, to: self).maxY - cropRect.minY
            let width = !keepAspectRatio ? cropRect.size.width : cropRect.size.width * (cappedHeight / cropRect.size.height)
            cropRect.size = CGSize(width: width, height: cappedHeight)
        }
    ...
    fileprivate func setCropAspectRatio(_ ratio: CGFloat, shouldCenter: Bool) {
        var cropRect = myScrollView.frame
        ...
        zoomToCropRect(cropRect, shouldCenter: shouldCenter, animated: false) {
            let scale = self.myScrollView.zoomScale
            self.myScrollView.minimumZoomScale = scale
        }
    }
